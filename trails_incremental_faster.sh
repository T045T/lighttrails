...
VIDEO_OUT=$(basename $VIDEO_IN .mp4)_${TRAIL}f_rollinger.mp4

CURDIR=$PWD
TEMP=$(mktemp -d)

# Split video into single frames
ffmpeg -i $VIDEO_IN -vframes 60 ${TEMP}/out-%d.png
cd ${TEMP}
...
for FRAME in $(seq $((${TRAIL}+1)) 50) #${NUMBEROFFRAMES})
do
	  echo -n "$FRAME "
    ...
    cp rolling-window.png old-window-${FRAME}.png
    convert -respect-parentheses \
            \( black-${REMOVE}.png -brightness-contrast -${FINAL_DARKENING}% -negate -write mpr:OLD -write tmp-${FRAME}.png \) \
            \( rolling-window.png mpr:OLD -compose ColorBurn -composite -write window-${FRAME}.png -write mpr:COMP \) \
            `# darken the remaining window` \
            \( mpr:COMP -brightness-contrast -${STEPSIZE}% -write mpr:X \) \
            ...
            null:
    composite -compose ColorBurn tmp-${FRAME}.png old-window-${FRAME}.png foo-${FRAME}.png
    mogrify -brightness-contrast -${STEPSIZE}% foo-${FRAME}.png
done
echo 'Done.'
